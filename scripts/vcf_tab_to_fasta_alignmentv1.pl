...
my $input_tab;
my $output_fas  = "output.fas";

my $usage = "usage: $0 [--exclude_het] [--output_ref] -i input.tab [-o output.fas]";
...
LINE: foreach my $line (<TAB>) {
	my @data = split /\t/, $line;
	$pos++;
	#print "number of position parsed: $pos\n";
...
my @nms = keys %hash;
for(my $i=0;$i<length($hash{$nms[0]});$i++) {
	my $ref = substr($hash{$nms[0]},$i,1);
	#print "R ",$i," ",$ref,"\n";
	my $good = 0;
	foreach my $j (sort {$a <=> $b} keys %hash){
		#print "A $i ",substr($hash{$j},$i,1),"\n";
		if(substr($hash{$j},$i,1) ne $ref) {
			$good = 1;
			last;
		}
	}
